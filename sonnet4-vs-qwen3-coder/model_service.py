...
async def get_model_response_async(model_name: str, prompt: str, context: Dict[str, Any]):
    ...
    try:
        ...
        response = await acompletion(
            ...
            messages=messages,
            api_key=os.getenv("OPENROUTER_API_KEY") if "coder" in model_name else os.getenv("ANTHROPIC_API_KEY"),
            max_tokens=2000,
            stream=True
        )
...
async def get_parallel_responses(prompt: str, context: Dict[str, Any]):
    ...
    openai_gen = get_model_response_async("qwen3-coder", prompt, context)
